...
| IPKT group path renamer | The standalone HTML remains self-contained and local-only, uses a compact letter-only Type selector with a legend and a live MQ line schematic, discovers arbitrary source groups, automatically uses each group's first source index as the hidden MQ anchor, checks every valid YXZ record for duplicates, supports P/G/Q/QL suffix rules, preserves source-index and coordinate-inferred MQ gaps, recognizes multiple separate bounded bridge spans, and automatically maps explicit `.EX` groups from the nearest configured prism/rail MQ using four positions normally and two positions beside bridge gaps. The editable EX output prefix updates both exports and the proposed schematic, which displays every calculated `MQxx-position`, missing EX positions, and reserved bridge MQs. It refuses an unanchored EX export instead of starting at MQ01, preserves skipped EX positions, applies direct-output Q/QL prism height offsets, replaces fixed-width fields as raw bytes, and exports normalized-path, final-MQ, rename-report, and separate duplicate-report files. |
| Group/path split build | `ipkt-group-path-renamer/index.html`, `style.css`, and `app.js` are canonical. The dedicated builder reproduces `IPKT-Group-Path-Renamer.html` exactly with inline assets and no external asset references. |
| GeoMonitoring interface | The group/path source and generated field file use the standard ink/teal design tokens, separate workflow stages, a local-processing trust indicator, button hierarchy, visible focus styles, 44 px touch targets, phone table guidance, and 320 px page-overflow protection. |
